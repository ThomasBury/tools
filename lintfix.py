...
# ///
"""
lintfix: fast, FOSS quality gate for Python repos.

- Formats (ruff), lints (ruff), type-checks (mypy), tests (pytest -n auto), audits (pip-audit).
- Initializes a minimal pyproject.toml if missing (Ruff + NumPy docstrings + mypy).
"""
...
def lint(
    ...
    """Lint & autofix with Ruff (NumPy docstyle). Optionally specify a file."""
    ensure_pyproject()
...
@app.command()
def test():
    """Run pytest (parallel on all cores)."""
    ensure_pyproject()
...
@app.command()
def all():
    """Run: format → lint → types → test → audit."""
    ensure_pyproject()
    steps = {
